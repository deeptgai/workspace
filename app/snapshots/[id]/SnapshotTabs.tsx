...
export function SnapshotTabs({ snapshot, evidenceMessages, people, actorname }: SnapshotTabsProps) {
  ...
  const [activeEvidence, setActiveEvidence] = useState<SnapshotEvidenceRef | null>(null);
  ...
  return (
    <>
            ...
            {filteredSignals.length ? (
              <div className="grid grid-cols-1 gap-2 md:grid-cols-2 xl:grid-cols-3">
                {filteredSignals.map((signal, index) => {
                  ...
                  return (
                    <article
                      className="group flex min-h-[190px] flex-col justify-between rounded-lg border border-slate-200 bg-white p-3 shadow-sm transition duration-200 hover:-translate-y-1 hover:border-emerald-200 hover:shadow-lg motion-safe:animate-[snapshotFadeIn_320ms_ease-out]"
                      key={signal.id}
                        ...
                        <div className="flex items-start justify-between gap-2">
                          ...
                          {firstEvidence ? (
                            <button
                              ...
                              type="button"
                              onClick={() => setActiveEvidence(firstEvidence)}
                            >
      ...
      {activeEvidence ? (
        <div className="fixed inset-0 z-50 grid justify-items-end bg-slate-950/45 backdrop-blur-sm" role="presentation" onClick={() => setActiveEvidence(null)}>
          <aside
            className="grid h-screen w-full max-w-3xl grid-rows-[auto_minmax(0,1fr)] border-l border-slate-200 bg-white shadow-2xl motion-safe:animate-[snapshotSlideIn_260ms_ease-out]"
            role="dialog"
            aria-modal="true"
          ...
          >
            <header className="flex items-start justify-between gap-4 border-b border-slate-200 bg-[#f9f6ed] p-4">
              <div>
                <h2 className="m-0 text-2xl font-black leading-tight text-slate-950">Сообщение #{activeEvidence.itemId}</h2>
                {activeMessage ? (
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    <span className="rounded-full bg-white px-2 py-1 text-xs font-black text-slate-600">{formatDateTime(activeMessage.publishedAt)}</span>
                    <span className="rounded-full bg-white px-2 py-1 text-xs font-black text-slate-600">{formatNumber(activeMessage.views)} views</span>
                    <span className="rounded-full bg-white px-2 py-1 text-xs font-black text-slate-600">{formatNumber(activeMessage.reactionsTotal)} reactions</span>
                    <span className="rounded-full bg-white px-2 py-1 text-xs font-black text-slate-600">score {activeMessage.engagementScore.toFixed(2)}</span>
                  </div>
                ) : null}
              </div>
              <button
            ...
            <div className="min-h-0 overflow-auto p-4">
              ...
              {activeMessage ? (
                ...
                <div className="rounded-lg border border-dashed border-slate-200 bg-white p-8 text-center text-slate-500">
                  Этого сообщения нет среди загруженных evidence-сообщений snapshot.
                </div>
              )}
